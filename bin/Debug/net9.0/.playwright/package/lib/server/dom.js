...
class ElementHandle extends js.JSHandle {
    ...
    async _clickablePoint() {
        ...
        const [quads, metrics] = await Promise.all([
            ...
            this._page.mainFrame()._utilityContext().then((utility) => utility.evaluate(() => ({
                width: innerWidth,
                height: innerHeight
            })))
        ]);
    ...
    async _performPointerAction(progress, actionName, waitForEnabled, action, forceScrollOptions, options) {
        ...
        const doScrollIntoView = async () => {
            ...
            return await this._scrollRectIntoViewIfNeeded(progress, position ? {
                x: position.x,
                y: position.y,
                width: 0,
                height: 0
        ...
        if (force) {
            ...
            const handle = await progress.race(this.evaluateHandleInUtility(([injected, node, {
    ...
    _hover(progress, options) {
        return this._retryPointerAction(progress, "hover", false, (point) => this._page.mouse.move(progress, point.x, point.y), {
            ...options,
            waitAfter: "disabled"
        });
    }
    ...
    _dblclick(progress, options) {
        return this._retryPointerAction(progress, "dblclick", true, (point) => this._page.mouse.click(progress, point.x, point.y, {
            ...options,
            clickCount: 2
    ...
    _tap(progress, options) {
        return this._retryPointerAction(progress, "tap", true, (point) => this._page.touchscreen.tap(progress, point.x, point.y), {
            ...options,
            waitAfter: "disabled"
        });
    }
    ...
    async _selectOption(progress, elements, values, options) {
        ...
        const result = await this._retryAction(progress, "select option", async () => {
            ...
            const result2 = await progress.race(this.evaluateInUtility(async ([injected, node, {
    ...
    async _fill(progress, value, options) {
        ...
        return await this._retryAction(progress, "fill", async () => {
            ...
            const result = await progress.race(this.evaluateInUtility(async ([injected, node, {
                value: value2,
                force
            }]) => {
                if (!force) {
    ...
    async _setInputFiles(progress, items) {
        ...
        const result = await progress.race(this.evaluateHandleInUtility(([injected, node, {
            multiple: multiple2,
            directoryUpload
        }]) => {
