...
class Frame extends import_instrumentation.SdkObject {
    ...
    async waitForSelector(progress, selector, performActionPreChecksAndLog, options, scope) {
        ...
        const promise = this.retryWithProgressAndTimeouts(progress, [0, 20, 50, 100, 100, 500], async (continuePolling) => {
            ...
            const {log, visible, attached} = await progress.race(result.evaluate((r) => ({
                log: r.log,
                visible: r.visible,
                attached: r.attached
            })));
    ...
    async click(progress, selector, options) {
        return dom.assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, !options.force, (handle) => handle._click(progress, {
            ...options,
        ...
        try {
            ...
            const result = await this.retryWithProgressAndTimeouts(progress, [100, 250, 500, 1e3], async (continuePolling) => {
                ...
                const {
                    matches,
                    received
                } = await this._expectInternal(progress, selector, options, lastIntermediateResult, false);
    ...
    async _expectInternal(progress, selector, options, lastIntermediateResult, noAbort) {
        ...
        const {log, matches, received, missingReceived} = await race(injected.evaluate(async (injected2, {
            info: info2,
            options: options2,
            callId
        }) => {
    ...
    async waitForFunctionExpression(progress, expression, isFunction, arg, options, world = "main") {
        ...
        return this.retryWithProgressAndTimeouts(progress, [100], async () => {
            ...
            const handle = await progress.race(injectedScript.evaluateHandle((injected, {
    ...
    async _callOnElementOnceMatches(progress, selector, body, taskData, options, scope) {
        ...
        const promise = this.retryWithProgressAndTimeouts(progress, [0, 20, 50, 100, 100, 500], async (continuePolling) => {
            ...
            if (!resolved)
                return continuePolling;
            const {log, success, value} = await progress.race(resolved.injected.evaluate((injected, {
                info,
