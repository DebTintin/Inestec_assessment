                ...
                import_fs.default.promises.readFile(file).then((buffer) => {
                    route.fulfill({
                        ...
                        headers: [
                            {
                                name: "Content-Type",
                                value: import_utilsBundle.mime.getType(import_path.default.extname(file)) || "application/octet-stream"
                            }
                        ],
    ...
    _updateActions(reveal) {
        ...
        for (const languageGenerator of (0, import_languages.languageSet)()) {
            const {
                ...
                actionTexts,
                text
            } = (0, import_language.generateCode)(actions, languageGenerator, this._languageGeneratorOptions);
            const source = {
                isRecorded: true,
